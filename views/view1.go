...
func View1(session neo4j.Session, params map[string]interface{}) {
	...
	MERGE (summary:%s_TYPE_SUMMARY {ip: $host_ip, entity: $entity, entity_type: $entity_type, detection_type: $detection_type, view: 1})
	ON CREATE SET summary.count = 1
	ON MATCH SET summary.count = summary.count + 1
	MERGE (e)-[:%s{view: 1}]->(summary)
	MERGE (nsummary:%s_NAME_SUMMARY {ip: $host_ip, entity: $entity, entity_type: $entity_type, name: $name, detection_type: $detection_type, view: 1})
