...
func View2(session neo4j.Session, params map[string]interface{}) {
	...
	query := fmt.Sprintf(`
	MERGE (e:ENTITY {ip: $host_ip, entity: $entity, view: 2})
	...
	query = `
	MATCH (e:ENTITY {ip: $host_ip, entity: $entity, view: 2})
	MERGE (ad:AS_DEST {ip: $host_ip, entity: $entity, view: 2, dest_ip: $dest_ip})
	...
	MERGE (sc:SEVERITY_CLUSTER {ip: $host_ip, entity: $entity, view: 2, dest_ip: $dest_ip, severity: $severity})
	ON CREATE SET sc.count = 1
	ON MATCH SET sc.count = sc.count + 1
	MERGE (ad)-[:HAS_SEVERITY]->(sc)
	
	MERGE (nc:NAME_CLUSTER {ip: $host_ip, entity: $entity, view: 2, dest_ip: $dest_ip, severity: $severity, name: $name})
	ON CREATE SET nc.count = 1
	ON MATCH SET nc.count = nc.count + 1
	MERGE (sc)-[:NAME_CLUSTER]->(nc)
	
	CREATE (a:ALERT {guid: $guid, timestamp: $timestamp, detection_type: $detection_type, name: $name, category: $category, mitre_tactic: $mitre_tactic, entity: $entity, host_ip: $host_ip, source_ip: $source_ip, dest_ip: $dest_ip, dest_port: $dest_port, dst_geo: $dst_geo, username: $username, syscall_name: $syscall_name, executable: $executable, process: $process, message: $message, proctitle: $proctitle, severity: $severity, view: 2})
	MERGE (nc)-[:INCLUDES]->(a)
	`
	res, err = session.Run(query, params)
	utils.HandleResult(res, err)
	...
	query = `
	MATCH (e:ENTITY {ip: $host_ip, entity: $entity, view: 2})
	MERGE (as:AS_SOURCE {ip: $host_ip, entity: $entity, view: 2, source_ip: $source_ip})
	...
	MERGE (scs:SEVERITY_CLUSTER {ip: $host_ip, entity: $entity, view: 2, source_ip: $source_ip, severity: $severity})
	ON CREATE SET scs.count = 1
	ON MATCH SET scs.count = scs.count + 1
	MERGE (as)-[:HAS_SEVERITY]->(scs)
	
	MERGE (ncs:NAME_CLUSTER {ip: $host_ip, entity: $entity, view: 2, source_ip: $source_ip, severity: $severity, name: $name})
	ON CREATE SET ncs.count = 1
	ON MATCH SET ncs.count = ncs.count + 1
	MERGE (scs)-[:NAME_CLUSTER]->(ncs)
	
	CREATE (aa:ALERT {guid: $guid, timestamp: $timestamp, detection_type: $detection_type, name: $name, category: $category, mitre_tactic: $mitre_tactic, entity: $entity, host_ip: $host_ip, source_ip: $source_ip, dest_ip: $dest_ip, dest_port: $dest_port, dst_geo: $dst_geo, username: $username, syscall_name: $syscall_name, executable: $executable, process: $process, message: $message, proctitle: $proctitle, severity: $severity, view: 2})
